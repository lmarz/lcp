	...
	while(ptr != NULL) {
		...
		if(ptr->status == 0x04 && ti >= ptr->tout) {
			...
			if(ptr->count > 3) {
				...
				/*
				 * TODO: Maybe close the proxy-link, instead of
				 * letting it time-out.
				 */
			...
			/* Add encryption-key if it should be enabled */
			tmp = LCP_HDR_SIZE;
			if((hdr.flg & LCP_F_ENC) == LCP_F_ENC) {
		...
		if(ptr->status == 0x08 && ti >= ptr->tout) {	
			...
			if(ptr->count > 3) {
				...
				/* Otherwise just remove connection */
				lcp_con_remv(ctx, &ptr->addr);
				goto next;
			}
		...
		if(ptr->status == 0x0d && ti >= ptr->tout) {
			...
			/* Failed to send hint */
			if(ptr->count > 3) {
				lcp_push_evt(ctx, LCP_HNTFAILED, ptr->slot, 
						&ptr->addr, NULL, 0);
